...
router.put('/comment', requireLogin, async (req, res) => {
  ...
  try {
    const postCommented = await Post.findByIdAndUpdate(postId, {
      $push: {comments: comment}
    }, { new: true }).populate('postedBy', '_id name').populate('comments.postedBy', 'id name');
...
router.delete('/delete/:id', requireLogin, async (req, res) => {
  const { id } = req.params;
  try {
    const deletedPost = await Post.findOne({ _id: id }).populate("postedBy", "_id");
    if (deletedPost) {
